...
def arias_energy_duration(data, data2):
	...
	F = abs(np.subtract(N1_final,N2_final))
	c = 10*(1-max(F))
	return c
#------------------------testing-------------------------------
#acc1 = (1.1,2.2,3.3)
...
if __name__ == "__main__":
	...
	#reading signals from two recent files
	list_of_files = glob.glob('/Users/anuja/Desktop/testing/signals/*') 
	recent_data = sorted(list_of_files, key=os.path.getmtime)
	
	try:
		time, dis, vel, acc = np.loadtxt(recent_data[-2] ,skiprows=1,unpack = True);
		time2, dis2, vel2, acc2 = np.loadtxt(recent_data[-1],skiprows=1,unpack = True);
	except IOError:
	...
	x1 = x.tolist()
	#header = np.chararray((c1, c2 ,c3, c4, c5, c6, c7))
	#print x1
	#print x
	...
	file_name = os.path.join(save_path, 'scores' + str(actual_time)+".txt")   

	np.savetxt(file_name, x1 , delimiter=' ',comments="",fmt='%f')
